    ...
    if (categorySelect) {
        categorySelect.addEventListener('change', function(e) {
            ...
            if (category) {
                ...
                document.getElementById('moviesContainer').innerHTML = '<div class="col-span-full text-center py-8 text-gray-400">Vui lòng chọn thể loại để xem phim</div>';
...
async function loadMoviesByCategory(category, page = 1) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            showError('Không thể tải danh sách phim cho thể loại này');
        }
    } catch (error) {
...
// Display movies (reuse from index.js)
function displayMovies(movies) {
    ...
    if (!movies || movies.length === 0) {
        container.innerHTML = '<div class="col-span-full text-center py-8 text-gray-400">Không có phim nào trong thể loại này</div>';
        return;
    }
...
// Display pagination (reuse from index.js)
